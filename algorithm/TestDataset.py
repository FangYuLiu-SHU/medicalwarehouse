...
class TestDataset2(data.Dataset):
    ...
    def __init__(self, randomList, cursor):
        # 读数据
        self.data_x, self.data_y, self.data_id, self.proper_color, self.moss_color = [], [], [], [], []#将舌色、苔色一并放进去，方便直接映射提取对应的标签
        lable_df = pd.read_csv('./files/merge_pulse_tongue.csv')
        ...
        for i in randomList:
            ...
            try:
                ...
                for j in range(N):
                    self.data_y.append(lable_df['pulse2'][i])
                    self.data_id.append(lable_df['id'][i])
                    self.proper_color.append(str(lable_df['tongue_proper_color'][i]))
                    self.moss_color.append(str(lable_df['tongue_moss_color'][i]))
                    self.data_x.append(pulse_df.loc[j * (row / N):(j + 1) * (row / N) - 1, :].values)
            except:
                print('从服务器获取数据失败')
    ...
    # 取第几个数据
    def __getitem__(self, index):
        return self.data_x[index], self.data_y[index], self.data_id[index], self.proper_color[index], self.moss_color[index]
